document.addEventListener('DOMContentLoaded', async () => { 
    ...
    let lastQuery = '';
    let hasError = false;
    let isVocabularyLoaded = false; // New flag to track vocabulary.json status
    ...
    async function loadData() {
        try {
            ...
            isVocabularyLoaded = true; // Set the flag to true once the data is successfully loaded
            return true;
        } catch (error) {
    ...
    function loadSearchFromHash() {
        if (!isVocabularyLoaded) return; // Ensure vocabulary is loaded before using this function
    ...
    window.addEventListener('load', async () => {
        ...
        if (isLoaded) {
            loadSearchFromHash(); // Only load search from hash if vocabulary.json is loaded
        }
    });

    window.addEventListener('hashchange', () => {
        if (!isVocabularyLoaded) return; // Ensure vocabulary is loaded before updating hash
    ...
    searchBox.addEventListener('input', (e) => {
        if (!isVocabularyLoaded) return; // Ensure vocabulary is loaded before responding to input
    ...
    function createClickableWords() {
        Object.keys(clickableWords).forEach(word => {
            ...
            resultDiv.innerHTML = resultDiv.innerHTML.replace(regex, `<span class="clickable-word">$1</span>`);
        });
